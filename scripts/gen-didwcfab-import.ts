/**
 * Parse the DIDWCFAB (Double Inlet Double Width, Forward Curve) performance
 * catalog into the two CSVs the admin Import screen accepts:
 *   scripts/out/didwcfab-catalogue.csv  (import as "catalogue" FIRST)
 *   scripts/out/didwcfab-ratings.csv    (import as "ratings" AFTER the catalogue)
 *
 * Like the FC catalog, each model is its own tab ("0900DIDWCFAB", …). Each tab
 * holds a CFM × static-pressure grid (each SP split into RPM and BHP). We emit
 ...
 *
 * DIDWCFAB is forward-curve double-width and is tagged DIDWCFAB. Its body price
 * base is the nearest CEB size's catalogue price; the ÷0.9 (forward) × ÷0.57
 * (double-width) factor and material factor are applied at quote time, not baked
 * into the stored base price. Its sizes (9"–36.5") align 1:1 with CEB sizes.
 *
 * Run: npx tsx scripts/gen-didwcfab-import.ts
 */
...
const SRC = join(process.cwd(), "DIDWCFAB Catalog.xlsx");
const OUT_DIR = join(process.cwd(), "scripts", "out");

// CEB catalogue (blade Ø in, price) from "Categories for Claude.xlsx" — the ×1
// price base. Each DIDWCFAB size adopts the nearest CEB size's price.
...
/** `.5"`, `1"`, `1.25"`, `2.5"`, `1/2"`, `1-1/4"` -> inches w.g. */
...
async function main() {
  ...
  for (const ws of wb.worksheets) {
    ...
    const tm = tab.match(/^(\d+)DIDWCFAB$/i);
    if (!tm) continue;
    const code = tm[1];
    const dia = Number(code) / 100;
    const modelCode = `AV${code}DIDWCFAB`;
  ...
  if (!models.length) throw new Error("No <size>DIDWCFAB tabs found");
  ...
  const catRows = models.map((m) => {
    ...
    const name = `Centrifugal Blower - DIDW ${m.sizeLabel}\" Forward Curved (DIDWCFAB)`;
    return [
      m.modelCode,
      "CENTRIFUGAL",
      csv(name),
  ...
  writeOut("didwcfab-catalogue.csv", [catHeader, ...catRows].join("\n") + "\n");
  ...
  writeOut("didwcfab-ratings.csv", [ratHeader, ...ratRows].join("\n") + "\n");
  ...
  console.log(`Wrote scripts/out/didwcfab-catalogue.csv and scripts/out/didwcfab-ratings.csv`);
}
