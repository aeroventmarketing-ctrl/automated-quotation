/**
 * Parse the HPB (Centrifugal High Pressure Blower) performance catalog into the
 * two CSVs the admin Import screen accepts:
 *   scripts/out/hpb-catalogue.csv  (import as "catalogue" FIRST)
 *   scripts/out/hpb-ratings.csv    (import as "ratings" AFTER the catalogue)
 *
 * Each model is its own tab ("1650HPB", …) named by wheel diameter ×100
 * (1650 = 16.50"). Each tab holds a CFM × static-pressure grid where every SP
 ...
 *
 * HPB is backward-curved and tagged HPB. Its size codes align 1:1 with CEB; the
 * stored base price is the matching CEB size's catalogue price, and the ×2.5
 * (= ÷0.4) High Pressure factor is applied at quote time via TAG_FACTORS["HPB"].
 ...
 *
 * Run: npx tsx scripts/gen-hpb-import.ts
 */
...
const SRC = join(process.cwd(), "HPB Catalog.xlsx");
const OUT_DIR = join(process.cwd(), "scripts", "out");

// CEB catalogue (blade Ø in, price) — the price base (same table as CIEB). Each
// HPB size adopts the matching/nearest CEB size's price; ×2.5 is applied at quote time.
...
async function main() {
  ...
  for (const ws of wb.worksheets) {
    ...
    // Locate the RPM/BHP header row; the row above holds the SP labels.
    let hdrRow = 0;
  ...
  if (!models.length) throw new Error("No <size>HPB tabs found");
  ...
  const catRows = models.map((m) => {
    ...
    const name = `High Pressure Blower ${m.sizeLabel}\" Backward Curved (HPB)`;
    return [
      m.modelCode,
      "CENTRIFUGAL",
      csv(name),
  ...
  writeOut("hpb-catalogue.csv", [catHeader, ...catRows].join("\n") + "\n");
  ...
  writeOut("hpb-ratings.csv", [ratHeader, ...ratRows].join("\n") + "\n");

  console.log("Models:");
  for (const m of models) {
    console.log(
      `  ${m.modelCode}  Ø${m.sizeLabel}"  area ${m.outletArea_ft2} ft²  maxRPM ${m.maxRpm}  ` +
        `base ₱${m.basePrice} (CEB Ø${m.cebDia}") ×2.5 = ₱${Math.round(m.basePrice / 0.4)}  ${m.points.length} pts`,
    );
  }
  console.log(`\nTotal rating points: ${total}`);
  console.log(`Wrote scripts/out/hpb-catalogue.csv and scripts/out/hpb-ratings.csv`);
}
