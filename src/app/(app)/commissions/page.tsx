...
import { VoucherSelection, type SelectableDeal } from "./voucher-selection";
import { CommissionMonths, type SearchableMonth } from "./commission-months";
import { dealHaystack } from "@/lib/commission-search";
...
export default async function CommissionsPage() {
  ...
  /**
   * The rows, with the four answers only this module can give.
   *
   * `lib/sales-commission` reaches Prisma, so the browser cannot ask it whether
   * a deal is voucherable or payable today — it is told. That keeps the search
   * box able to HIDE a row and nothing else: it can never make one payable, put
   * one on a voucher, or change what it is worth.
   */
  const searchableMonths: SearchableMonth[] = months.map((m) => ({
    ...m,
    deals: m.deals.map((d) => ({
      ...d,
      key: dealKey(d),
      voucherable: isVoucherable(d),
      payNow: canMarkPaid(d, todayYMD),
      payOpensYMD: markPaidOpensYMD(d.payoutYMD),
      haystack: dealHaystack(d.refLabel, d.company),
    })),
  }));
        ...
        <>
          ...
          {months.length === 0 ? (
            ...
            <VoucherSelection deals={selectableDeals} currency={currency} todayYMD={todayYMD}>
              <CommissionMonths
                months={searchableMonths}
                currency={currency}
                quotaGross={MONTHLY_QUOTA_GROSS}
                ratePct={COMMISSION_RATE_PCT}
                overrideRatePct={OVERRIDE_RATE_PCT}
                canManage={canManage}
