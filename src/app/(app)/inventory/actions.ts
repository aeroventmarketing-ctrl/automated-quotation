...
/**
 * Move stock to another location. A full move with no existing stock at the
 * destination just relocates the item; otherwise the quantity is issued from the
 * source and received into the destination record (an existing same-name record
 * there, or a new one). Reserved stock can't be moved. Both sides get a ledger
 * movement so the transfer is auditable.
 */
export async function transferStock(input: z.infer<typeof transferSchema>): Promise<void> {
  const user = await requireInventoryManager();
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (d.qty > available) {
      throw new Error(`Only ${available} ${item.unit} available to transfer${reserved > 0 ? " (the rest is reserved)" : ""}.`);
    }
    const noteSuffix = d.note ? ` · ${d.note}` : "";

    // Existing stock record for the same item at the destination.
    const dest = await tx.stockItem.findFirst({
      where: { active: true, id: { not: item.id }, name: { equals: item.name, mode: "insensitive" }, location: { equals: toLocation, mode: "insensitive" } },
    });

    // Full move, nothing at the destination yet → just relocate in place.
    if (!dest && d.qty === onHand) {
      await tx.stockItem.update({ where: { id: item.id }, data: { location: toLocation } });
      await tx.stockMovement.create({
        data: { stockItemId: item.id, kind: "ADJUSTMENT", delta: 0, balanceAfter: onHand, reason: `Transferred ${fromLocation || "—"} → ${toLocation}${noteSuffix}`, byName: user.name },
      });
      return;
    }

    // Split move: issue from the source…
    const srcBalance = round3(onHand - d.qty);
    await tx.stockItem.update({ where: { id: item.id }, data: { quantity: srcBalance } });
    await tx.stockMovement.create({
      data: { stockItemId: item.id, kind: "ISSUE", delta: -d.qty, balanceAfter: srcBalance, reason: `Transfer to ${toLocation}${noteSuffix}`, byName: user.name },
    });

    // …and receive into the destination (merge into existing, or create new).
    if (dest) {
      const destBalance = round3(Number(dest.quantity) + d.qty);
      await tx.stockItem.update({ where: { id: dest.id }, data: { quantity: destBalance } });
      await tx.stockMovement.create({
        data: { stockItemId: dest.id, kind: "RECEIPT", delta: d.qty, balanceAfter: destBalance, reason: `Transfer from ${fromLocation || "—"}${noteSuffix}`, byName: user.name },
      });
    } else {
      const sku = await nextSku(tx);
      const created = await tx.stockItem.create({
        data: { sku, name: item.name, unit: item.unit, category: item.category, location: toLocation, quantity: d.qty, reorderLevel: item.reorderLevel, unitCost: item.unitCost },
      });
      await tx.stockMovement.create({
        data: { stockItemId: created.id, kind: "RECEIPT", delta: d.qty, balanceAfter: d.qty, reason: `Transfer from ${fromLocation || "—"}${noteSuffix}`, byName: user.name },
      });
    }
  });
  revalidatePath("/inventory");
