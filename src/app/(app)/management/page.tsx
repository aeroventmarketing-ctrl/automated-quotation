...
export default async function ManagementPage() {
  ...
  let scheduleCalendars: string[] = ["General"];
  ...
  try {
    ...
    scheduleUsers = allUsers;
    scheduleCalendars = cals;
    const base = rows.map((s) => buildScheduleView(s, { viewerId: viewer?.id, canDecide: canApproveSchedule }));
    scheduleRows = expandOccurrences(base, since.getTime(), Date.now() + 365 * 86_400_000);
  } catch {
