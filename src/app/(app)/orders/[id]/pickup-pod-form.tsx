...
import { saveCloseDoc, removeCloseDoc, approvePickupDelivery } from "../actions";
...
/**
 * Office pickup: Sales uploads the proof of pick up (into the "pod" slot) and then
 * approves it in one step — marking the pickup a successful delivery. Mirrors
 * DeliveredForm but is Sales-facing and calls approvePickupDelivery.
 */
export function PickupPodForm({ orderId, initialFiles, admin = false }: { orderId: string; initialFiles: SaleDoc[]; admin?: boolean }) {
  ...
  async function approve() {
    setBusy(true); setErr(null);
    try {
      await approvePickupDelivery(orderId);
      router.refresh();
    } catch (e) {
      setErr(e instanceof Error ? e.message : "Failed");
    } finally { setBusy(false); }
  ...
  return (
    <div className="space-y-2">
      <Label className="text-xs">Proof of pick up (signed documents / photos)</Label>
      ...
      <Button size="sm" disabled={busy || files.length === 0} onClick={approve}>{busy ? "Saving…" : "Approve POP - Successful Pick Up"}</Button>
      {files.length === 0 && <p className="text-[11px] text-muted-foreground">Attach the proof of pick up to enable “Approve POP - Successful Pick Up”.</p>}
