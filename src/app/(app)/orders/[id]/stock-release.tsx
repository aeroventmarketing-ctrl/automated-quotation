...
import { releaseOrderFromStock } from "../actions";

/**
 * Phase 2 card for a from-stock order (in-house duct hardware — angle corner,
 * cleats, clips — nothing fabricated or bought from a supplier). The Warehouse or
 * Fans & Blowers head matches each line to a stock item and releases it: inventory
 * is deducted and the order jumps to Phase 5. Mirrors the MRF release picker.
 */
    ...
    <div className="space-y-3">
      <div className="rounded-md border border-sky-300 bg-sky-50 p-3 text-sm dark:border-sky-900 dark:bg-sky-950/30">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          purchase order. Issue them from inventory to fulfil the order; it then moves to final payment.
        </p>
      ...
      {!canRelease ? (
        <p className="text-xs text-muted-foreground">
          Awaiting the Warehouse / Fans &amp; Blowers to release the stock.
        </p>
      ) : open ? (
        <StockMatchPanel
