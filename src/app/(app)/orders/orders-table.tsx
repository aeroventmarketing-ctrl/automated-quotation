...
export interface OrderRow {
  ...
  /** Job Orders generated by the Engineer (per department), for the Purchaser view. */
  jobOrders: { number: string; dept: string }[];
}
...
export function OrdersTable({
  ...
  restricted?: boolean;
  /** Purchaser view: hide Value/Collected/Balance, show a Job Orders column instead. */
  isPurchaser?: boolean;
  initialStage?: string;
  ...
  // Purchaser view collapses Value/Collected/Balance (3 cols) into one Job Orders col.
  const colSpan = isPurchaser ? 8 : 10;
        ...
        <Table>
          ...
          <TableBody>
            {sorted.length === 0 ? (
              ...
              groups.map((g) => (
                <GroupRows key={g.key} groupKey={g.key} rows={g.rows} showHeader={group !== "none"} currency={currency} progressHidden={progressHidden} restricted={restricted} isPurchaser={isPurchaser} colSpan={colSpan} />
              ))
            )}
          </TableBody>
        </Table>
...
function GroupRows({ groupKey, rows, showHeader, currency, progressHidden, restricted, isPurchaser, colSpan }: { groupKey: string; rows: OrderRow[]; showHeader: boolean; currency: string; progressHidden: boolean; restricted: boolean; isPurchaser: boolean; colSpan: number }) {
    ...
    <>
      ...
      {rows.map((o) => (
        <TableRow key={o.id}>
          ...
          {isPurchaser ? (
            <TableCell>
              {o.jobOrders.length === 0 ? (
                <span className="text-muted-foreground">—</span>
              ) : (
                <div className="flex flex-wrap gap-x-2 gap-y-0.5">
                  {o.jobOrders.map((jo) => (
                    <Link
                      key={`${jo.dept}-${jo.number}`}
                      href={`/orders/${o.id}`}
                      title={`${jo.dept} job order — open the JO workflow`}
