...
import { SaleDocReader } from "./sale-doc-reader";
...
export function SalePanel({
  ...
  // Accounting (canClear) and the Admin / Payment Approver (unlimited) may run
  // the closing-document reader, not just the sale editor. The read / approve /
  // limit policy lives in <SaleDocReader>.
  ...
  const renderDocSlot = (type: SaleDocType) => {
    ...
    // Sales Invoice / Collection Receipt / Delivery Receipt are AI-read (captured
    // + verified) for VAT-inclusive / zero-rated deals — a pure VAT-exclusive
    // deal issues an Acknowledgement / Delivery Form instead, which isn't read.
    ...
    return (
      <div key={type.key} className="space-y-1">
        ...
        {readable && files.length > 0 && (
          <SaleDocReader
            quotationId={quotationId}
            docKey={type.key}
            files={files.map((f) => f.doc)}
