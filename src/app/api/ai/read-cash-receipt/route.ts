...
PH SALES-INVOICE BOOKLET — "SALES INVOICE" or "CHARGE SALES INVOICE", handwritten OR pre-printed (a padded form the supplier fills in — e.g. WINGS COMMERCIAL (handwritten), GOLDEN PACIFIC INC., TKL STEEL CORPORATION, ALLOYMASTER INDUSTRIAL SUPPLY, IDEAL CONTROLS INC., INTERNATIONAL SPRING INDUSTRIES, JSL ELECTRIC CORPORATION, and any dealer/hardware/industrial-supply sales-invoice booklet with a "SALES INVOICE"/"CHARGE SALES INVOICE" header, a "Sold to"/"Registered Name" block, and a body table with columns QUANTITY | (UNIT/UOM) | ARTICLES/ITEM DESCRIPTION | UNIT PRICE | AMOUNT). Whether handwritten or typed, the figures on a SUPPLIER sales invoice ARE the official values — READ THEM (unlike bank deposit slips). Rules:
  • INVOICE NUMBER: read the pre-printed serial next to "No." / "Invoice No." at the TOP-RIGHT (a 4–8 digit serial, usually red — e.g. "47019", "16443", "954314", "11790", "35817", or with leading zeros like "0001877"), exactly as printed (keep leading zeros), into "invoiceNumber".
  • DATE: the "DATE:" near the top — M/D/YY or D-M-YY (e.g. "8/12/26" or "4-17-26" = 17 Apr 2026) or written in full (e.g. "May 13, 2026" / "12-Aug-26"). Ignore the tiny BIR "Date Issued" at the very bottom (the booklet's printing date).
  ...
  • AMOUNT SPENT — use the VAT-INCLUSIVE GROSS TOTAL, i.e. the "Total Sales (VAT Inclusive)" figure (equivalently VATable Sales + VAT Amount, = the sum of the body "AMOUNT" column). Set "receiptTotal" to it. TWO TRAPS — do NOT use either: (a) a middle "AMOUNT DUE" / "Amount Net of VAT" line (that is NET of VAT, VAT-exclusive, lower by 12%); (b) a "Total Amount Due" that has "Less: Withholding Tax" (EWT) SUBTRACTED — the withholding is a creditable tax remitted to BIR (2307), not a cost reduction, so ALWAYS use the full VAT-inclusive gross (e.g. TKL Steel: Total Sales (VAT Inclusive) 12,840.00, "Less: Withholding Tax 11.64" → "Total Amount Due 12,725.36" — use 12,840.00; JSL Electric: 16,295.00 not 16,149.51). When there is no withholding line, the "Total Amount Due" equals the VAT-inclusive gross. Each matched line's "actualAmount" must sum to that VAT-inclusive gross.
