...
SUPPLIER SALES INVOICE (e.g. TOZEN PHILIPPINES, INC. — SAP Business One "SALES INVOICE" format, and similar supplier VAT invoices): the AMOUNT is the "Amount Due" in the totals box at the bottom-right — the VAT-inclusive payable (VATable + VAT Amount). Use "Amount Due" as the total spent; do NOT use the per-line "Total Amount"/"VATable" subtotal, and do NOT use the "VAT Amount", "WTax"/EWT, "Discount" or "Zero Rated" rows on their own. The DATE is the "Date" in the Document Reference box (the invoice date) — not "Delivery Date", "Terms", or the "Date Issued" at the very bottom.
