...
PH SALES-INVOICE BOOKLET — handwritten OR pre-printed (a padded "SALES INVOICE" form filled in by the supplier — e.g. WINGS COMMERCIAL (handwritten), GOLDEN PACIFIC, INC. (typed/printed), and any dealer/hardware/industrial-supply sales-invoice booklet with a "SALES INVOICE" header, a "SOLD TO"/"Registered Name" block, and a body table whose columns are QUANTITY | (UNIT) | ARTICLES/DESCRIPTION | UNIT PRICE | AMOUNT). Whether the entries are handwritten or typed, the figures on a SUPPLIER sales invoice ARE the official values — READ THEM (unlike bank deposit slips). Rules:
  • INVOICE NUMBER: the pre-printed serial next to "No." at the TOP-RIGHT (typically 4–6 digits, printed in RED or black — e.g. "16443"). Read that number (digits only, drop the leading "No.") into "invoiceNumber".
  • DATE: the "DATE:" near the top — handwritten M/D/YY (e.g. "8/12/26" = 12 Aug 2026) or typed in full (e.g. "August 11, 2026"). Report it in "date" as written. (Ignore the tiny pre-printed "Date Issued" / BIR "Date Issued" at the very bottom of the form — that's the booklet's printing date, not the sale date.)
  • LINES: read each body row — QUANTITY, the ARTICLES/DESCRIPTION text (e.g. "\"ASAHI\" BRAND BEARING UNITS UCF PILLOW BLOCK UCF208-24"), UNIT PRICE, and AMOUNT (= quantity × unit price). Match each to a PO line by description/meaning.
  • AMOUNT PAID: use the GRAND-TOTAL row in the bottom-right totals box — labelled "TOTAL AMOUNT DUE" or "TOTAL AMT. DUE" (bottom-most, alongside "E. & O.E.") — set "receiptTotal" to it. These invoices are VAT-INCLUSIVE ("Total Sales" is marked "VAT Inclusive"): set "vatMode" to "inclusive". CRITICAL TRAP: do NOT use a middle "AMOUNT DUE" / "Amount Net of VAT" line — on a VAT-inclusive PH invoice that is the amount NET of VAT (e.g. it shows AMOUNT DUE 1,339.29 then "Add: VAT 12% 160.71" → TOTAL AMT. DUE 1,500.00; the payable is 1,500.00, NOT 1,339.29). Likewise ignore "Less: VAT", "Less: Withholding Tax", "VATable Sales", "Zero-rated", and "VAT Amount" rows. Each matched line's "actualAmount" must sum to the GRAND TOTAL (for a single line, it equals the grand total); use the line AMOUNT column to split across multiple lines, then scale so the actuals sum to the grand total.
