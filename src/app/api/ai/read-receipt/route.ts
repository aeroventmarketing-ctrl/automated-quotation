...
PH SALES-INVOICE BOOKLET — handwritten OR pre-printed (a padded "SALES INVOICE" form filled in by the supplier — e.g. WINGS COMMERCIAL (handwritten), GOLDEN PACIFIC, INC. and TKL STEEL CORPORATION (typed/printed), and any dealer/hardware/industrial-supply sales-invoice booklet with a "SALES INVOICE" header, a "Sold to"/"Registered Name" block, and a body table whose columns are QUANTITY | (UNIT) | ARTICLES/ITEM DESCRIPTION | UNIT PRICE | AMOUNT). Whether the entries are handwritten or typed, the figures on a SUPPLIER sales invoice ARE the official values — READ THEM (unlike bank deposit slips). Rules:
  • INVOICE NUMBER: the pre-printed serial next to "No." / "Invoice No." at the TOP-RIGHT (typically 4–6 digits, printed in RED or black — e.g. "16443", "954314"). Read that number (digits only, drop the leading "No.") into "invoiceNumber".
  • DATE: the "DATE:" near the top — handwritten M/D/YY (e.g. "8/12/26" = 12 Aug 2026) or typed (e.g. "August 11, 2026" or "12-Aug-26"). Report it in "date" as written. (Ignore the tiny pre-printed "Date Issued" / BIR "Date Issued" at the very bottom of the form — that's the booklet's printing date, not the sale date.)
  • LINES: read each body row — QUANTITY, the ARTICLES/ITEM DESCRIPTION text (e.g. "NATIONBOND #18 (1.1)"), UNIT PRICE, and AMOUNT (= quantity × unit price). Match each to a PO line by description/meaning.
  • AMOUNT PAID — use the VAT-INCLUSIVE GROSS TOTAL, i.e. the "Total Sales (VAT Inclusive)" figure (equivalently VATable Sales + VAT Amount, and equal to the sum of the body "AMOUNT" column). Set "receiptTotal" to it and "vatMode" to "inclusive". This is the goods value and it matches the PO/voucher. TWO CRITICAL TRAPS — do NOT use either:
      ...
      (b) a "Total Amount Due" / "Total Amt. Due" that has had "Less: Withholding Tax" (EWT) SUBTRACTED — the withholding tax is a creditable tax remitted to BIR (the BIR 2307), NOT a reduction in the goods cost, so the reconciliation amount is still the full VAT-inclusive gross. Example (TKL Steel): Total Sales (VAT Inclusive) 12,840.00, then "Less: Withholding Tax 11.64" → "Total Amount Due 12,725.36"; use 12,840.00, NOT 12,725.36. When there is NO withholding line, the "Total Amount Due" equals the VAT-inclusive gross — either reads the same.
