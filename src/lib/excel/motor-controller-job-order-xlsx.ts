/**
 * Builds a Motor Controller Job Order workbook from scratch with ExcelJS — a
 * branded header, the JO number + due date, then a table of motor-controller
 * lines (qty, starter/control method, HP, phase, voltage), followed by the note.
 */
...
export async function buildMotorControllerJobOrderWorkbook(jo: MotorControllerJobOrder): Promise<Buffer> {
  ...
  doctype.value = "MOTOR CONTROLLER JOB ORDER";
  ...
  if (jo.lines.length === 0) {
    ...
    cell.value = "No motor controllers listed.";
  ...
  const noteLines = wrapNote(jo.note || "", 68);
  ...
  ws.getCell(`E${r}`).border = { top: { style: "thin", color: { argb: "FF000000" } } };
