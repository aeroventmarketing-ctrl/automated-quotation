...
/**
 * Resolve a user's printed name + uploaded signature from a stored NAME (many
 * documents keep only the signatory's name, not their id). Matches a user by
 * name and returns their signature, or null when there's no such user / no
 * signature on file.
 */
export async function resolveSignatureByName(name: string | null | undefined): Promise<{ name: string; signature: string | null }> {
  const nm = (name ?? "").trim();
...
export async function resolvePurchaserSignature(name: string | null | undefined): Promise<{ name: string; signature: string | null }> {
  return resolveSignatureByName(name);
}
